...
const validate = {
  ...
  surname: (value) => {
    return value.trim().length < 3
      ? {
          surname: true,
          surnameError: "Last Name must be atleast 3 characters long.",
        }
  ...
  password: (value) => {
    return passwordRegex.test(value)
      ...
      : {
          password: true,
          passwordError:
            "Minimum 8 characters, 1 uppercase, 1 lowercase, 1 symbol (@$%#^&*) and 1 number (0-9).",
        };
  },
  loginPassword: (value) => {
    return !value.trim()
  ...
  dob: (value) => {
    ...
    if (dob.getFullYear() + 10 < today.getFullYear()) {
      ...
      return {
        dob: true,
        dobError: "Invalid Date-of-Birth. Age must be atleast 10 years.",
      };
    }
  },
};
